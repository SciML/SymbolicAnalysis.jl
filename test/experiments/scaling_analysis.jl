...
function run_part1_scaling()
    ...
    return (
        karcher = (nodes=karcher_nodes, times_ns=karcher_times_ns, alpha=alpha_k, R2=R2_k),
        tyler = (nodes=tyler_nodes, times_ns=tyler_times_ns, alpha=alpha_t, R2=R2_t),
        scalar = (nodes=scalar_nodes, times_ns=scalar_times_ns, alpha=alpha_s, R2=R2_s),
    )
end
...
function run_part2_phase_decomposition()
    ...
    @printf("  %-4s  %6s  %10s  %10s  %10s  %10s  %10s\n",
        "m", "nodes", "canonize", "sign", "curvature", "gcurvature", "total")
    @printf("  %-4s  %6s  %10s  %10s  %10s  %10s  %10s\n",
        "", "", "(us)", "(us)", "(us)", "(us)", "(us)")
    println("  " * "-" ^ 68)
    ...
    for m in term_counts
        ...
        @printf("  m=%2d  %5d  %10.1f  %10.1f  %10.1f  %10.1f  %10.1f\n",
            m, nn,
            t_canon / 1e3, t_sign / 1e3, t_curv / 1e3, t_gcurv / 1e3,
            total / 1e3)

        push!(phase_data, (
            m=m, nodes=nn,
            canon_ns=t_canon, sign_ns=t_sign, curv_ns=t_curv, gcurv_ns=t_gcurv,
            total_ns=total
        ))
    end
    println()
    ...
    @printf("  propagate_gcurvature:%5.1f%%  <-- DGCP marginal cost\n", 100 * last.gcurv_ns / dgcp_total)
    println()
...
function run_part3_memory()
    ...
    if length(mem_nodes) >= 3
        ...
        @printf("Bytes per AST node: %.0f - %.0f (range)\n",
            minimum(bytes_per_node), maximum(bytes_per_node))
    end
    println()
...
function run_part4_conic()
    ...
    return (nodes=conic_nodes, times_ns=conic_times_ns, epi_vars=conic_epi, constraints=conic_cons)
end
...
function run_part5_summary_table(part1, part2, part3, part4)
    ...
    if !isempty(part2)
        ...
        @printf("  %-24s  %10.1f  %7.1f%%\n", "DCP total (3 phases)", dcp_only / 1e3, 100 * dcp_only / total)
    ...
    for i in eachindex(part4.nodes)
        @printf("  %5d  %10.1f  %8d  %11d\n",
            part4.nodes[i], part4.times_ns[i] / 1e3,
            part4.epi_vars[i], part4.constraints[i])
    end
    println()
    ...
    println("  " * "-" ^ 50)
    @printf("  %-30s  %8.3f  %8.4f\n", "Karcher (DGCP, full)", part1.karcher.alpha, part1.karcher.R2)
    @printf("  %-30s  %8.3f  %8.4f\n", "Tyler (DGCP, full)", part1.tyler.alpha, part1.tyler.R2)
    @printf("  %-30s  %8.3f  %8.4f\n", "Scalar (DCP, full)", part1.scalar.alpha, part1.scalar.R2)
...
function run_part6_matrix_independence()
    ...
    @printf("Node count range: %d - %d (%.1fx)\n",
        minimum(nodes_vec), maximum(nodes_vec),
        maximum(nodes_vec) / minimum(nodes_vec))
    @printf("Time range: %.1f - %.1f us (%.1fx)\n",
        minimum(times_vec) / 1e3, maximum(times_vec) / 1e3, time_range)
...
function main()
    ...
    println("*" ^ 72)
    println()
    @printf("Configuration: matrix_dim=%d, warmup=%d, timing_iters=%d\n",
        MATRIX_DIM, WARMUP_ITERS, TIMING_ITERS)
