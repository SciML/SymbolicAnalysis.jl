...
function run_and_save_timing()
    ...
    for (name, expr, both) in cases
        dcp_t = time_verification(7) do
            analyze(expr)
        end
        dgcp_t = time_verification(7) do
            analyze(expr, M)
        end
...
function run_and_save_benchmark()
        ...
        for sz in sizes
            ...
            push!(
                ...
                (
                    ...
                    Nodes = nodes,
                    Depth = depth,
                    Memory_KB = alloc / 1024,
                ),
            )
            println(
                @sprintf(
                    "  %-15s %3dx%-3d  %.3f ms  %3d nodes  depth %d",
                    ptype,
                    sz,
                    sz,
                    t,
    ...
    ax = Axis(
        fig[1, 1],
        title = "Verification Time vs AST Nodes",
        xlabel = "AST Nodes",
        ylabel = "Time (ms)",
    )
    ...
    println(
        "  → Saved extended_benchmark.csv, benchmark_nodes_vs_time.png, benchmark_size_vs_time.png",
    )
    println()
    return df
end
...
function run_and_save_expert()
    ...
    cases = [
        ...
        ("Spectral Fn", "Hard", SymbolicAnalysis.eigsummax(log(X), 2)),
    ]
    ...
    for (name, difficulty, expr) in cases
        ...
        push!(
            ...
            (
                ...
                Result = string(r.gcurvature),
                Time_ms = t_ms,
            ),
        )
        println(
            @sprintf("  %-20s [%s] → %s  (%.2f ms)", name, difficulty, r.gcurvature, t_ms)
        )
    end
...
function run_and_save_mle()
    ...
    for (n, m) in [(3, 3), (3, 5), (3, 10), (5, 3), (5, 5), (5, 10)]
        ...
        println(
            @sprintf(
                "  Frechet n=%d m=%2d: DGCP=%.4fs DCP=%.4fs gcvx=%s",
                n,
                m,
                dgcp_t,
                dcp_t,
    ...
    for (n, k) in [(3, 3), (3, 5), (5, 3), (5, 5)]
        ...
        println(
            @sprintf(
                "  Tyler   n=%d k=%2d: DGCP=%.4fs DCP=%.4fs gcvx=%s",
                n,
                k,
                dgcp_t,
                dcp_t,
